...
function sanitizeVegaSvg(svgText) {
...
function looksLikeVegaSpecText(text) {
    ...
    const hasVegaLiteShape =
        lower.includes('"encoding"') &&
        (lower.includes('"mark"') || lower.includes('"layer"') || lower.includes('"hconcat"') || lower.includes('"vconcat"')) &&
        lower.includes('"data"');
    const hasVegaShape =
        lower.includes('"marks"') &&
        (lower.includes('"scales"') || lower.includes('"axes"') || lower.includes('"signals"'));
    if (hasVegaLiteShape || hasVegaShape) return true;
    return false;
}
...
function normalizeVegaLiteTopLevelFold(spec) {
    if (!spec || typeof spec !== 'object' || Array.isArray(spec)) return false;
    if (!Object.prototype.hasOwnProperty.call(spec, 'fold')) return false;

    const isLikelyVegaLite = Boolean(spec.encoding || spec.mark || spec.layer || spec.concat || spec.hconcat || spec.vconcat)
        || (typeof spec.$schema === 'string' && spec.$schema.toLowerCase().includes('vega-lite'));
    if (!isLikelyVegaLite) return false;

    const topFold = spec.fold;
    let foldTransform = null;
    if (Array.isArray(topFold)) {
        foldTransform = { fold: topFold };
    } else if (topFold && typeof topFold === 'object' && !Array.isArray(topFold)) {
        foldTransform = { ...topFold };
    }
    if (!foldTransform) return false;

    const currentTransform = spec.transform;
    if (Array.isArray(currentTransform)) {
        spec.transform = [...currentTransform, foldTransform];
    } else if (currentTransform && typeof currentTransform === 'object') {
        spec.transform = [currentTransform, foldTransform];
    } else {
        spec.transform = [foldTransform];
    }
    delete spec.fold;
    return true;
}
    ...
    btn.addEventListener('click', async (event) => {
        ...
        try {
            ...
            const dpr = (typeof window !== 'undefined' && window.devicePixelRatio)
                ...
                : 1;
            const baseScale = 3;
            const scale = Math.min(4, Math.round(baseScale * dpr));
    ...
    const renderProcess = ensureVegaLite(loadScript).then(async (vegaEmbed) => {
        for (const codeElement of targets) {
            ...
            try {
                ...
                if (normalizedSpecText !== rawSpec) codeElement.dataset.vegaLiteSanitized = 'true';
            } catch (error) {
                if (isFinalRender) {
            ...
            try {
                ...
                normalizeVegaLiteTopLevelFold(specCopy);
                ...
                const renderedSvg = chartHost.querySelector('svg');
                if (renderedSvg) {
                    const sanitized = sanitizeVegaSvg(renderedSvg.outerHTML);
                    if (sanitized) {
                        chartHost.innerHTML = sanitized;
                    }
                }
